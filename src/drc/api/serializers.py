...
from drc.backend.exceptions import BackendException
...
class EnkelvoudigInformatieObjectSerializer(serializers.Serializer):
    url = serializers.URLField(max_length=200, read_only=True)
    identificatie = serializers.CharField(required=False, max_length=40, help_text='Een binnen een gegeven context ondubbelzinnige referentie naar het INFORMATIEOBJECT.')
    bronorganisatie = serializers.CharField(max_length=9, help_text='Het RSIN van de Niet-natuurlijk persoon zijnde de organisatie die het informatieobject heeft gecreëerd of heeft ontvangen en als eerste in een samenwerkingsketen heeft vastgelegd.')
    creatiedatum = serializers.DateField(help_text='Een datum of een gebeurtenis in de levenscyclus van het INFORMATIEOBJECT.')
    titel = serializers.CharField(max_length=200, help_text='De naam waaronder het INFORMATIEOBJECT formeel bekend is.')
    ...
    auteur = serializers.CharField(max_length=200, help_text='De persoon of organisatie die in de eerste plaats verantwoordelijk is voor het creëren van de inhoud van het INFORMATIEOBJECT.')
    ...
    taal = serializers.ChoiceField(choices=LANGUAGE_CHOICES, help_text='Een taal van de intellectuele inhoud van het ENKELVOUDIG INFORMATIEOBJECT. De waardes komen uit ISO 639-2/B')
    ...
    bestandsomvang = serializers.IntegerField(source='inhoud.size', read_only=True,min_value=0)
    ...
    informatieobjecttype = serializers.URLField(max_length=200, help_text='URL naar de INFORMATIEOBJECTTYPE in het ZTC.')
